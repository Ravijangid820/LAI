...
def test_urgency_buckets(make_llm_json, stub_rag) -> None:
    ...
    make_llm_json(
        [
            {"kind": "expired", "date": (today - timedelta(days=10)).isoformat(), "description": "past due"},
            {"kind": "urgent", "date": (today + timedelta(days=15)).isoformat(), "description": "soon"},
            {"kind": "soon", "date": (today + timedelta(days=60)).isoformat(), "description": "two months"},
            {"kind": "future", "date": (today + timedelta(days=400)).isoformat(), "description": "next year"},
        ]
    )
    out = extract_timeline(doc_ids=["d1"], full_text="")
...
def test_non_iso_date_passes_through_without_urgency(make_llm_json, stub_rag) -> None:
    ...
    make_llm_json(
        [
            {"kind": "construction_milestone", "date": "Q4 2026", "description": "Tower delivery"},
        ]
    )
    out = extract_timeline(doc_ids=["d1"], full_text="")
    assert len(out) == 1
...
def test_empty_date_drops_row(make_llm_json, stub_rag) -> None:
    ...
    make_llm_json(
        [
            ...
            {"kind": "y", "date": (date.today() + timedelta(days=10)).isoformat(), "description": "real"},
        ]
    )
    out = extract_timeline(doc_ids=["d1"], full_text="")
    assert len(out) == 1
...
def test_dict_wrapped_response(make_llm_json, stub_rag) -> None:
    make_llm_json(
        {
            "timeline": [
                {"kind": "x", "date": (date.today() + timedelta(days=1)).isoformat(), "description": "wrapped"},
            ]
        }
    )
    out = extract_timeline(doc_ids=["d1"], full_text="")
