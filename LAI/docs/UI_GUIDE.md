...
## Status snapshot — 2026-05-17 audit

A lot of this is already shipped. Confirmed by reading
...
| `ConfidentialityBadge` | ✅ **just landed** | `components/ConfidentialityBadge.tsx` |
| `LanguageToggle` + `LanguageProvider` | ✅ **just landed** | `components/LanguageToggle.tsx` + `contexts/LanguageContext.tsx` |
| `ragApi.ts` types (`Chunk.cite_id`, `source_kind`, `citation_validation`) | ✅ shipped | `lib/ragApi.ts` |
| **PDF preview for `[M-n]` in the side panel** | ❌ blocked on backend `GET /document/{session_id}` | the `CitationPanel.tsx` comment at line 8-10 explicitly flags this |
| **`target_language` actually drives the answer language** | ⚠️ frontend ready, backend `POST /query` doesn't yet accept the field | see `LanguageContext.tsx` for the backend-side change list |
| **SSE streaming on `/query`** | ❌ backend non-streaming today | half-day backend task |
| Demo seed Matter ("Windpark Lamstedt") | ❌ not yet | half-day product+seed work |

**TL;DR:** ~80% of the frontend is done. Three remaining items, two of
them backend-gated:

1. PDF preview in the matter side panel — needs `GET /document/{session_id}` backend addition first.
2. `target_language` wiring — needs `POST /query` to accept the new field.
3. Demo seed Matter — needs the curated PDFs + a tiny seed script.
...
| Day | Frontend deliverables | Depends on backend |
|---|---|---|
| Day 2 | `<CitationChip>`, `<UnverifiedBadge>`, `<AssistantMessage>` renderer. Wire `/query` → render chips. Click chip → side panel with chunk text. | ✅ backend already shipping `cite_id` + `source_kind` |
| Day 2 | Side panel for `M-n`: `react-pdf` opened to page 1 of the uploaded PDF | ✅ backend stores upload bytes; serve a `GET /document/{session_id}` to fetch the PDF (small backend addition, ~30 min) |
| Day 2 | Thinking indicator on chat turn (Option A above) | none |
| Day 3 | `<LanguageToggle>` wired to a new `target_language` field in `/query` | ⚠️ requires a backend change to accept `target_language` and inject it into `RAG_SYSTEM` (half-day backend) |
| Day 4 | `<ConfidentialityBadge>` (static), `<UnverifiedBadge>` styling polish | none — `(unbelegt)` markers already in the response |
| Day 5 | Sidebar `<DocumentList>` with status dots, multi-doc support | ⚠️ backend currently allows one upload per session; the multi-doc Matter model is a backend Day-5 task |
| Day 6 | `<DropZone>` drag-and-drop, document status polling, parsed/embedded/indexed indicators | ⚠️ backend Day-6: `GET /session/{id}/documents` to list, status field per doc |
| Day 7 | (auth — deferred to v1.1 per strategy doc §11.2) | — |
| Day 8 | Empty-state example questions, demo-seed Matter pre-loaded ("Windpark Lamstedt") | ⚠️ backend Day-8: seed script + a fixed `demo` session_id the UI opens by default |
| Day 9 | Loading skeletons, error states, polish, demo rehearsal | none |
| Day 10 | Final polish, demo | none |
