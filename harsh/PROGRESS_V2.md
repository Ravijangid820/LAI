...
- **mode-router-quality** — ✅ **SCOPED 2026-06-02** in blueprint [`rj/blueprint/2026-06-02-mode-router-quality.md`](../rj/blueprint/2026-06-02-mode-router-quality.md) (`1e3cb0a`). Two-phase plan: Phase 1 (½ day) extends `serve_rag.CONVERSATIONAL` regex with the observed UI/meta patterns ("was kann ich hier tun", "wie funktioniert das", "gehst du semantisch vor"), unit-tested AND sanity-checked against vm-9's `bimschg_50.jsonl` gold-RAG set so it can't eat a legitimate legal question. Phase 2 (2–3 days, optional) is a k-NN intent classifier over query embeddings against a labelled probe set, built only if Phase 1's regex turns out brittle on paraphrases. Eval gate: `ui_meta_recall ≥ 0.7` AND `content_precision` within 1 pp of baseline. **Recommendation:** ship Phase 1 in a spare half-day after pilot lands; Phase 2 only if pilot feedback says paraphrase failures persist.
