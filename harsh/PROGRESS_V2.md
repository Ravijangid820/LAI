...
- 🚨 **val.jsonl gold quality is the real Recall@K ceiling** (`5bf1024`, 2026-06-02). New `LAI/scripts/eval/inspect_misses.py` (`223f4a4`) hand-audited 25 hybrid-baseline misses. **52 % of misses have questionable or unrelated gold** — 28 % `gold_unrelated` (Danish/English text for German legal questions, metadata tables for "Rechtsgebiet" questions) + 24 % `gold_questionable` (gold sort-of fits but the retrieved top-1 is at least as good). Concrete example: "Welche steuerlichen Auswirkungen Verkauf Anteilsholding" — gold is a Danish balance table; the model's retrieved top-1 is the **correct German § 8b KStG tax-law commentary**, but the harness scored this as a miss. Adjusted Recall@30 estimate is **0.63–0.75**, not 0.49. The published 0.49 was an honest measurement of a partially-broken target. **Reorders the next-experiments list:** val re-curation is now first, candidate_k bump second, query rewriting third. Full audit + verdict-by-verdict table + implications at [`rj/blueprint/2026-06-02-val-quality-finding.md`](../rj/blueprint/2026-06-02-val-quality-finding.md).
