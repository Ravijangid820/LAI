...
- **mode-router-quality** — ✅ **Phase 1 SHIPPED 2026-06-02** (`0f4ce4d`). New `UI_META` regex in [`serve_rag.py:1054-1077`](../LAI/src/lai/api/serve_rag.py#L1054) checked BEFORE `LEGAL_KEYWORDS` so a legal word in a meta question ("wie funktioniert das mit BImSchG?") still routes to chat. 25/25 unit tests pass — 21 UI/meta phrasings caught (German UI/navigation + German meta-AI + English mirrors), 1 safety check confirms every BImSchG legal question in vm-9's gold set escapes the regex, 3 integration checks on `needs_rag()`. Blueprint at [`rj/blueprint/2026-06-02-mode-router-quality.md`](../rj/blueprint/2026-06-02-mode-router-quality.md) (`1e3cb0a`). **Phase 2 (embedding-based intent classifier, 2-3 days) deferred** until pilot feedback shows the regex is brittle on paraphrases.
