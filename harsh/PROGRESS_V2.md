...
- ✅ **BM25 variant dispatcher + sweep tool** (`c83d690`, 2026-06-02) — `LAI_BM25_VARIANT` env switches between 6 expressions (v1 control, v2 OR-4, v3 OR-3, v5 stopword filter, v6 prefix-glob, v7 length-routed AND/OR) without forking the code path; serve_rag stays on v1 because the env is never set. `LAI/scripts/eval/bm25_variant_sweep.py` drives the harness once per variant and applies the blueprint decision rule (drop any -1 pp R@30 loss; pick lowest retrieve_ms survivor). v1 OR-of-6 matches 1.99M FTS5 rows; v2 1.59M; v3 967k; v6 prefix-glob 10.8M (slower but recall-tilted); v7 just 2 rows on a long German query (AND branch). Verified expressions execute against the live 35.7M-row FTS5 index. **Sweep in flight** — chained background job running both the ef=200 hybrid confirmation AND the variant sweep at n=200, hybrid mode. Results land at `rj/blueprint/2026-06-02-retrieval-tuning-results.md` (placeholder) and follow-up commits.
