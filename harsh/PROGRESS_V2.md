...
- ✅ **val.jsonl language filter SHIPPED** (`e0f4bb4` + `edbf740`, 2026-06-02). New `LAI/scripts/eval/filter_val_german.py` (CLI) + `lai.search.val_language.classify_text` (library, 12 unit tests). Hard-rejects on Danish letters/tokens or English-dominated text without umlauts; keeps only positive German signal. Full run on all 9998 val rows kept **8429 (84.3 %) German**, dropped 1072 non_de + 497 unknown. The first 500 rows of val.jsonl happened to be Danish-heavy (42.4 % kept) — explains why the original baseline ran against a contaminated subset. Filtered output at `val_de.jsonl` (gitignored). Re-run hybrid against the German-only set is the next step to surface the "real" Recall@K number.
