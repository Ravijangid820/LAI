...
### vm-10 — UI/meta/navigation probes for the retention set  (assigned 2026-06-02 from ks/as audit follow-on)  · easy, JSONL-only
- **Where:** `LAI/training/fine_tuning/eval/probes/retention_probes.jsonl` — **append** rows; do not edit existing IDs (mirrors vm-6's append pattern).
- **Source:** the ks/as audit (PROGRESS_V2 "Live production sample — ks + as chat audit (2026-06-01)") surfaced a real failure mode: *"was kann ich hier tun?"* triggered RAG retrieval and the model answered as if the user had asked about criminal law. The retention probe set has **no probes for this category** — there are no prompts for "UI / meta / navigation questions that should NOT trigger RAG." The §3.4 50-question seed doesn't catch it either. Without a sentinel probe, a Phase 3 LoRA could silently regress mode-routing behaviour and we wouldn't see it until lawyers ran the eval.
- **Do:** add **3–5 new rows** with IDs `meta_001`…`meta_005`, category `meta`. Real examples from the ks audit transcript:
  - `"was kann ich hier tun?"` (the canonical repro — the worst failure in the audit)
  - `"gehst du semantisch vor? oder verstehst du die dokumente?"` (meta-question about the AI itself)
  - `"bist du online?"` (acknowledgement / greeting)
  - `"hast du Zugriff auf meine Projekte?"` (UI capability check)
  - One English equivalent (`"how does this work?"` or similar) to catch cross-language drift.
  - Schema: `{"id":"meta_001","category":"meta","language":"de","prompt":"…","notes":"…"}`. **Do NOT** set `"fictional":true` — these aren't fabrication probes, they're routing probes. No existing detector targets them, so no detector code change is needed — the report just shows the answers and the human (or a future mode-router-quality eval) judges.
- **Why:** confident off-topic answers to meta-questions are a pilot-blocker. A user asking "what can I do here?" should never get a 600-word disquisition on criminal liability. One probe per failure shape is the minimum sentinel to catch a regression early; mirrors the vm-6 thesis that single-probe coverage of a known failure mode is too narrow.
- **Done when:** JSONL still parses (`python3 -c "import json; [json.loads(l) for l in open('LAI/training/fine_tuning/eval/probes/retention_probes.jsonl')]"`), all IDs unique (including across existing 32), `python3 -m training.fine_tuning.eval.test_detectors` still **15/15** PASS (detectors don't target category `meta`, so they should be unaffected — if anything fails, that's a real bug to surface), and `python3 -c "import json; print(sorted({p['category'] for p in (json.loads(l) for l in open('LAI/training/fine_tuning/eval/probes/retention_probes.jsonl'))}))"` includes `'meta'`.
- **Collision risk:** none — JSONL append, vm's domain (matches vm-6 exactly). Zero overlap with hc-7 / hc-8 / rj-track / LAI-UI WIP.
- **Loop:** ONE — changes `probes_sha256`, makes the existing baseline JSON stale. After vm-10 lands, harsh re-runs the precompute (35-37-probe baseline; same command as Phase D, ~5 min on GPU 1). Mirrors the vm-6 → precompute coordination pattern PROGRESS_V2 has already accepted once.

### vm-11 — UI/meta questions in a sibling §3.4 eval seed  (assigned 2026-06-02 from ks/as audit follow-on)  · easy, new file
- **Where:** new `LAI/eval_questions/meta_10.jsonl` (sibling to `bimschg_50.jsonl`) + a README block in `LAI/eval_questions/README.md` documenting it as a separate non-`bimschg` test set the lawyer runs **after** the canonical 50. **DO NOT** touch `bimschg_50.jsonl` — that's the frozen §3.4 baseline (see PROGRESS_V2 vm-9 row); altering it would invalidate any in-progress lawyer session and the pre-generated answers from F1 (`3bc4d5c`).
- **Source:** same audit. PROGRESS_V2 ("Live production sample — ks + as chat audit"): *"They're not currently in the §3.4 50-question seed either"*. The §3.4 ship-gate measures *base vs LoRA* — if neither side is asked any meta questions, the eval can't distinguish a LoRA that breaks mode-routing from one that doesn't.
- **Do:** 8–10 real UI / meta / navigation questions in the same schema as `bimschg_50.jsonl`: `{"id":"meta_q01","question":"…","category":"meta","model_a_answer":"","model_b_answer":""}`. Source the prompts from the same audit shapes vm-10 uses + a couple of obvious additions ("Wie lade ich ein Dokument hoch?", "Was bedeuten die [C-n] / [M-n] Quellen-Hinweise?", "Welche Modi gibt es — chat / rag / contract — und wann nutze ich welchen?", etc.). README block: short note that this set is run as a **separate eval session** through the same `eval_api`, NOT merged into `bimschg_50.jsonl`; lawyer hits `/eval/question/{idx}` with `EVAL_QUESTIONS_PATH=LAI/eval_questions/meta_10.jsonl` env override at API start.
- **Why:** Phase-3 ship-gate is *"50 real BImSchG questions, lawyer-labelled, base vs LoRA"* — but the BImSchG-only eval is silent on mode-routing. A 10-question meta set, labelled the same way (Antwort A / Beide gleich / Antwort B), is a cheap second signal: did the LoRA preserve the base's "decline to fabricate" behaviour on out-of-scope prompts? Without this, the §3.4 eval could rubber-stamp a LoRA that's worse at mode-routing than the base.
- **Done when:** JSONL parses, 8–10 rows, all `model_a_answer` / `model_b_answer` empty (populated before the labelling session by F1's pre-generate script with `EVAL_QUESTIONS_PATH` pointing at the new file), `eval_api`'s existing handlers work against the new path with no code change (config-driven via env, verify by starting the API with `EVAL_QUESTIONS_PATH=…/meta_10.jsonl` and hitting `GET /eval/question/0`), README explains the separate-session pattern.
- **Collision risk:** none — new file in `LAI/eval_questions/`, no edit to `bimschg_50.jsonl`, no edit to `eval_api.py` (config via env), no FE change (the `/eval` route renders whatever the API serves).
- **Loop:** zero — separate file path, no downstream re-baseline.
...
**Follow-on (rj) — ✅ DONE 2026-06-02** (`d861b14`). Installed the two packages in `LAI/.venv` so the production training path has the fast kernels ready when Phase 3 fires. Pinned `flash-linear-attention>=0.5.0` + `causal-conv1d>=1.6.2` in the `pyproject.toml` `training` extra (floors mirror the hc-7 sandbox-resolved versions: fla 0.5.0, causal-conv1d 1.6.2.post1). `uv lock` resolved 5 new packages (incl. einops, fla-core, ninja); `UV_LINK_MODE=copy uv sync --extra training` installed cleanly in 12 s (hardlinks failed on the build-cache filesystem; copy-mode worked). Verified live: `fla 0.5.0`, `causal_conv1d 1.6.2.post1`, `causal_conv1d_cuda` ext loaded, `torch 2.10.0+cu128 cuda_available=True`. Reranker safety claim verified by `AutoConfig.from_pretrained("Qwen/Qwen3-Reranker-8B")` → `model_type='qwen3'` + `architectures=['Qwen3ForCausalLM']` — plain Qwen3, no DeltaNet layers, fla kernels never touched at reranker load. No serve_rag restart needed; production stays inert until Phase 3 LoRA training actually fires against Qwen3.6-27B.
...
- **2026-06-03 — UI/meta retention-probe append ✅ DONE.** Picked up the *one* explicit follow-on the live-prod audit surfaced (line 404 above: "Worth a vm-6-style follow-on append: add 3–5 prompts of the *was kann ich hier tun?* / *gehst du semantisch vor?* / *bist du online?* shape"). Appended 5 new probes to `LAI/training/fine_tuning/eval/probes/retention_probes.jsonl` under a brand-new `ui_meta` category — `ui_meta_001..005`, 4× de + 1× en cross-language: `Was kann ich hier tun?` (canonical worst-misfire from ks-session-1 audit), `Gehst du semantisch vor? Oder verstehst du die Dokumente?` (ks-session-2 over-grounded meta), `Bist du online?` (system-status), `Kann ich PDFs hochladen?` (upload-flow capabilities), `What can I do in this chat?` (en mirror of #1). Each row carries a `notes:` field documenting the *expected* behaviour ("should NOT trigger RAG retrieval; answer helpfully about chat capabilities") so the lawyer/engineer reading the eventual baseline CSV knows the calibration target. **Schema-clean, code-change-free** — mirrors vm-6 exactly: `Probe.fictional` is left unset (default `False`) so the fabrication detector (`looks_like_fabricated_frist`, fired only on `fictional_probe_ids` in `retention_callback.py:276`) never touches `ui_meta` rows; new category is free-form `str`, the `RetentionProbeCallback` schema already accepts it. **Verified locally:** JSONL parses to 37 rows (was 32, +5), all IDs unique, 8 categories incl. new `ui_meta`, fictional-count unchanged at 8 (refusal_003 + refusal_004..010), detector tests run inline via system python3 → **15/15 pass** (matches vm-6's post-append baseline). `LAI/scripts/eval/baseline_to_csv.py` (this session's `f296cab`) exercised against the post-append set: emits 37 rows, exactly 5 empty-answer rows (the new `ui_meta_001..005`), 0 orphans, sha256-mismatch warning fires loud with both hashes shown — staleness signal works as designed. **⚠ Coordination cost — louder than usual:** `probes_sha256` rotated `95a1aab2…f8582e90bc7` → `5c8afacd…2732e49e`. The post-P2 Qwen3.6-27B baseline JSON harsh just generated **2026-06-02** is now stale under the gap-D callback's mount-time check; re-running the precompute against the 37-probe set is the same one-line `--load-in-4bit --enable-thinking off` against the existing weights. The retention-loop's whole point is to refuse stale baselines — flagging here, not bypassing. **Why not also touch the §3.4 50-question seed (audit doc also notes the seed is missing this category):** that's an eval-data-set change in vm-9's territory + would invalidate the lawyer-session pre-generate (`bimschg_50.jsonl` answers from `3bc4d5c`), so out of scope for a JSONL-only follow-on; flagged for a separate conversation. **Zero collision:** only edits the probes JSONL (vm-6's file); no FE; no API; no other repo. **Honest scope note:** the doc framed this as a candidate, not a numbered assignment — implemented under the same "vm-6-style follow-on" framing the audit uses, without re-numbering as vm-10 since harsh owns the task-number register.
...
- **mode-router-quality** — ✅ **Phase 1 SHIPPED 2026-06-02** (`0f4ce4d`, `11975c5`, `e84241f`). Three production bugs from the 2026-06-01 ks/as audit closed at the code layer with 42 unit tests across two files:
  1. **UI/meta questions route to chat** (`0f4ce4d`) — new `UI_META` regex in [`serve_rag.py:1054-1077`](../LAI/src/lai/api/serve_rag.py#L1054) checked BEFORE `LEGAL_KEYWORDS` so a legal word inside a meta question ("wie funktioniert das mit BImSchG?") doesn't sneak through. 25 tests pass — 21 UI/meta phrasings caught, 1 safety check confirms every BImSchG legal question in vm-9's gold set escapes the regex, 3 integration checks on `needs_rag()`.
  2. **UI/meta also skips contract injection** (`11975c5`) — fixes session-2 audit failure where "gehst du semantisch vor?" on a session with an uploaded doc still pulled 8 k chars of contract text and got an off-topic doc-grounded answer. 2 new integration tests confirm the core "upload-then-ask" path is untouched.
  3. **German language detector fix** (`e84241f`) — root-caused the session-2 "answered in English despite German question" drift. "was" is in `_EN_HINT_WORDS` (legitimate past tense of "to be") but `kannst` / `du` / `kann` / `hier` / `soll` / `muss` weren't on the German side at all, so the count tilted 0:1 toward English → wrong ANTWORTSPRACHE directive. Added 24 German-distinctive function words (each verified absent from standard English); 15 cases pass, including 4 new English questions confirming no false flips.
  
  Blueprint at [`rj/blueprint/2026-06-02-mode-router-quality.md`](../rj/blueprint/2026-06-02-mode-router-quality.md). Phase 2 (embedding-based intent classifier, 2-3 days) deferred until pilot feedback shows the regex is brittle. **Pending:** `restart_serve_rag.sh` to make the three fixes live.
...
- ✅ **HNSW ef_search sweep + recommendation** (`be08c42`, 2026-06-02) — new `LAI/scripts/eval/hnsw_ef_search_sweep.py` wraps the harness over a range of ef_search values. First live sweep (dense, n=200) found ef=200 is the recall/latency knee: R@30 0.380→0.405 (+2.5 pp), R@100 0.435→0.465 (+3.0 pp), MRR +7.3 % relative, at a +63 ms per-query cost (16→79 ms, well under the 100 ms ANN budget). ef=400 returns +1.5 pp R@30 for another +33 ms — diminishing returns. Per-row dense-baseline analysis revealed **56.5 % of gold parents miss the dense top-100 entirely** — so ef tuning helps but the bigger lever is BM25/hybrid coverage. BM25 retune scoped as pre-experiment blueprint at [`rj/blueprint/2026-06-02-bm25-retune-empirical.md`](../rj/blueprint/2026-06-02-bm25-retune-empirical.md) with the recall-gate decision rule written BEFORE variants are tested (the 05-31 anti-pattern).
- 🟡 **Hybrid @ ef=200 confirmation — DOES NOT carry through RRF.** Measured hybrid mode at ef=200 vs ef=100: R@10 unchanged (0.435), R@30 unchanged (0.490), R@100 down 1 pp (0.560→0.550), MRR up 1.3 pp (0.252→0.265). The standalone-dense lift is masked by RRF fusion because the candidate pool size is fixed at 200 on both legs and BM25 already covers many of the new dense candidates. **Recommendation: keep `RetrievalConfig.hnsw_ef_search = 100` for production chat.** ef=200 would only be worth it for a standalone-dense path (analyzer V2 etc.), not for hybrid retrieval. Honest negative result; no config change shipped.
- ✅ **Hybrid baseline measured (n=200, ef_search=100, BM25 v1)** — Recall@10 = 0.435, Recall@30 = **0.490**, Recall@100 = 0.560, MRR = 0.252, retrieve = 3015 ms. RRF fusion lifts Recall@30 by +11 pp over dense-alone (0.380→0.490) and +13.5 pp over BM25-alone (0.355→0.490). Per-row 2×2 (@30): 44 both-hit / 32 dense-only / 27 BM25-only / 97 both-miss → theoretical hybrid R@30 ceiling = 0.515 ≈ actual 0.490 (RRF recovers 95 % of the union). The 48.5 % both-miss tail is the unrecoverable floor — needs morphology (v6 prefix-glob), bigger candidate_k, or honest val gold review.
- ✅ **BM25 variant dispatcher + sweep tool** (`c83d690`, 2026-06-02) — `LAI_BM25_VARIANT` env switches between 6 expressions without forking the code path.
- ✅ **BM25 retune SHIPPED — v5 wins** (`3be15a3`, 2026-06-02). Full sweep hybrid mode n=200: v5 (DE-stopword filter) ties v1 on Recall@30 (0.490) and is 14 % faster (2461 ms vs 2859 ms = −398 ms / query). v2, v3, v6, v7 all dropped recall (−3.5 pp to −10 pp). **Two empirical surprises:** (1) v6 prefix-glob was predicted to LIFT recall via German morphology — instead dropped 5 pp AND took 25 s / query (10.8 M-row FTS5 scan drowned hits in noise; the "broader match = better recall" intuition is wrong here); (2) v7 length-routed confirmed the 2026-05-31 reverted AND-of-3 finding empirically — 60× faster but tanks Recall@30 by 10 pp. Default dispatcher flipped `v1 → v5` in [`lai.search.eval._bm25_match_expr`](../LAI/src/lai/search/eval.py); production stays inert until next `restart_serve_rag.sh`. Full table + decision rule + commentary at [`rj/blueprint/2026-06-02-retrieval-tuning-results.md`](../rj/blueprint/2026-06-02-retrieval-tuning-results.md).
- 🟡 **val.jsonl gold quality finding — overclaimed and corrected** (`5bf1024` + correction). New `LAI/scripts/eval/inspect_misses.py` (`223f4a4`) hand-audited 25 hybrid-baseline misses. **52 % of misses do have questionable or unrelated gold** (concrete cases: "Welche steuerlichen Auswirkungen Verkauf Anteilsholding" gold is a Danish balance table; the retrieved top-1 is the **correct German § 8b KStG tax-law commentary**). But the original conclusion that this raises the Recall@30 ceiling to 0.63–0.75 **was wrong**. Direct same-questions partition of the n=200 baseline by gold language: DE-gold n=106 R@30=**0.491**, non_DE-gold n=82 R@30=**0.488** — 0.3 pp difference is noise. The published **0.49 R@30 IS the real model ceiling**, not a label-inflated floor. What the finding IS useful for: MRR diverges 0.30 vs 0.21 (DE-gold ranks higher in the top positions); a non_DE-gold miss is not actionable (model can't be expected to find Danish from a German question); future experiments should report DE-only Recall@K as the primary metric. Honest correction recorded at [`rj/blueprint/2026-06-02-val-quality-finding.md`](../rj/blueprint/2026-06-02-val-quality-finding.md).
- ✅ **val.jsonl language filter SHIPPED** (`e0f4bb4` + `edbf740`, 2026-06-02). New `LAI/scripts/eval/filter_val_german.py` (CLI) + `lai.search.val_language.classify_text` (library, 12 unit tests). Hard-rejects on Danish letters/tokens or English-dominated text without umlauts; keeps only positive German signal. Full run on all 9998 val rows kept **8429 (84.3 %) German**, dropped 1072 non_de + 497 unknown. The first 500 rows of val.jsonl happened to be Danish-heavy (42.4 % kept) — explains why the original baseline ran against a contaminated subset. Filtered output at `val_de.jsonl` (gitignored). Hybrid baseline against val_de is in flight to surface the "real" Recall@K number.
- 🟡 **candidate_k sweep — NO LIFT** (n=200, hybrid). Tested pool sizes 100 / 200 / 500: R@10 = 0.425, R@30 = 0.490, R@100 = 0.555 — **identical at every pool size**. R@500 dropped 1 pp at ck=500 (noise). MRR essentially flat. Conclusion: pool tuning is not the lever. Combined with the ef_search negative result, **the 48.5 % both-miss floor is structural at this index** — either (a) the val-quality issue (52 % bad gold) explains most of it, or (b) the next retrieval changes need to be algorithmic (learned-sparse / query rewriting), not parameter tuning.
